    ...
    <script>
                ...
                try {
                    ...
                    if (result.success) {
                        if (result.affected_rows === 0) {
                            showToastNotification('No records were updated. You may not be assigned as a signatory for the College sector\'s clearance period.', 'warning');
                        } else {
        ...
        async function loadCurrentPeriod() {
            try {
                ...
                // Check if there's an active period specifically for "College" sector
                let hasActivePeriod = false;
                if (data.success && data.active_periods && data.active_periods.length > 0) {
                    // Check if any active period is for College sector
                    hasActivePeriod = data.active_periods.some(period => 
                        period.sector === 'College' || 
                        period.clearance_type === 'College'
                    );
                    
                    if (hasActivePeriod) {
                        const period = data.active_periods.find(p => 
                            p.sector === 'College' || 
                            p.clearance_type === 'College'
        ...
        async function approveSignatory(userId) {
            ...
            if (!canPerformSignatoryActions) {
                showToastNotification('You are not assigned as a signatory for the College sector\'s clearance period. Approving clearances is not available.', 'warning');
                return;
            }
        ...
        async function rejectSignatory(userId) {
            // Guard: Check if user is assigned as signatory
            if (!canPerformSignatoryActions) {
                showToastNotification('You are not assigned as a signatory for the College sector\'s clearance period. Rejecting clearances is not available.', 'warning');
                return;
            }
            ...
            // Get signatory_id from row for fetching existing rejection details
