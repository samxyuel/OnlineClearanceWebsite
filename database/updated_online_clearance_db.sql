...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 13, 2025 at 07:52 PM
...
--
-- Stand-in structure for view `active_clearance_periods`
-- (See below for the actual view)
--
CREATE TABLE `active_clearance_periods` (
`period_id` int(11)
...
CREATE TABLE `semesters` (
  ...
  `semester_name` enum('1st','2nd') NOT NULL,
  `academic_year_id` int(11) DEFAULT NULL,
  `is_active` tinyint(1) DEFAULT 0 COMMENT 'Only one can be active at a time',
...
--
-- Structure for view `active_clearance_periods`
--
DROP TABLE IF EXISTS `active_clearance_periods`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `active_clearance_periods`  AS SELECT `cp`.`period_id` AS `period_id`, `cp`.`academic_year_id` AS `academic_year_id`, `cp`.`semester_id` AS `semester_id`, `cp`.`sector` AS `sector`, `cp`.`status` AS `status`, `cp`.`start_date` AS `start_date`, `cp`.`end_date` AS `end_date`, `ay`.`year` AS `academic_year`, `s`.`semester_name` AS `semester_name` FROM ((`clearance_periods` `cp` join `academic_years` `ay` on(`cp`.`academic_year_id` = `ay`.`academic_year_id`)) join `semesters` `s` on(`cp`.`semester_id` = `s`.`semester_id`)) WHERE `cp`.`status` in ('Ongoing','Paused') ;
