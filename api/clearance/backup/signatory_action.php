...
try {
    ...
    if ($get->fetch()) {
        if ($action === 'Approved') {
            ...
            $upd = $pdo->prepare("UPDATE clearance_signatories SET action=?, remarks=NULL, reason_id=?, additional_remarks=?, updated_at=NOW(), actual_user_id=NULL, signatory_first_name=NULL, signatory_last_name=NULL, date_signed=NULL WHERE clearance_form_id=? AND designation_id=?");
        ...
        $firstNameToInsert = ($action === 'Approved') ? $signerFirstName : null;
        $lastNameToInsert = ($action === 'Approved') ? $signerLastName : null;
