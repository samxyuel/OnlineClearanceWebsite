-- =====================================================
-- ONLINE CLEARANCE WEBSITE - COMPLETE DATABASE SCHEMA
-- =====================================================
-- This schema supports all implemented features and future enhancements
-- Created based on comprehensive requirements analysis
-- =====================================================
...
CREATE TABLE `roles` (
  ...
  `role_name` VARCHAR(50) UNIQUE NOT NULL COMMENT 'Admin, School Administrator, Program Head, Regular Staff, Student, Faculty',
  `description` TEXT,
...
-- =====================================================
-- PHASE 2: CLEARANCE MANAGEMENT
-- =====================================================

-- Clearance periods (one active per academic year + semester)
...
-- Clearance requirements (what each user type needs)
CREATE TABLE `clearance_requirements` (
  `requirement_id` INT PRIMARY KEY AUTO_INCREMENT,
  `clearance_type` ENUM('Student', 'Faculty') NOT NULL,
  `department_id` INT NULL COMMENT 'NULL means all departments',
  ...
  INDEX `idx_clearance_requirements_department` (`department_id`),
...
-- =====================================================
-- PHASE 3: USER TYPE SPECIFIC TABLES
-- =====================================================
...
CREATE TABLE `program_heads` (
  `program_head_id` INT PRIMARY KEY AUTO_INCREMENT,
  `user_id` INT UNIQUE,
  `department_id` INT,
  `assigned_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `is_active` BOOLEAN DEFAULT TRUE,
  ...
  INDEX `idx_program_heads_department` (`department_id`),
  INDEX `idx_program_heads_active` (`is_active`)
);
...
CREATE TABLE `school_administrators` (
  ...
  INDEX `idx_school_admins_designation` (`designation_id`)
);
...
CREATE TABLE `notification_templates` (
  ...
  `template_name` VARCHAR(100) NOT NULL COMMENT 'e.g., "Clearance Approved", "Rejection Notice"',
  `template_type` ENUM('email', 'sms', 'push', 'in_app') NOT NULL,
  `subject` VARCHAR(255) COMMENT 'Email subject line',
  `body` TEXT NOT NULL COMMENT 'Template body with placeholders',
  `variables` JSON COMMENT 'Available placeholder variables',
  `is_active` BOOLEAN DEFAULT TRUE,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
('School Administrator', 'School-wide clearance management'),
('Program Head', 'Department-specific clearance management'),
('Regular Staff', 'Staff with signatory responsibilities'),
...
SELECT 'Online Clearance Website Database Schema Created Successfully!' AS message;
