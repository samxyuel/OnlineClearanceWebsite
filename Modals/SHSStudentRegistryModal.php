...
window.openStudentRegistrationModal = function() {
  console.log('[SHSStudentRegistryModal] ===== openStudentRegistrationModal called =====');
  
  try {
    const modal = document.getElementById('studentRegistrationModal');
    console.log('[SHSStudentRegistryModal] Modal element search result:', modal);
    
    if (!modal) {
      console.error('[SHSStudentRegistryModal] ❌ Modal element not found!');
    ...
    console.log('[SHSStudentRegistryModal] ✅ Modal element found');
    console.log('[SHSStudentRegistryModal] Modal current display:', window.getComputedStyle(modal).display);
    console.log('[SHSStudentRegistryModal] Modal current opacity:', window.getComputedStyle(modal).opacity);
    console.log('[SHSStudentRegistryModal] Modal inline style:', modal.style.display);

    // Use window.openModal if available, otherwise fallback
    if (typeof window.openModal === 'function') {
      console.log('[SHSStudentRegistryModal] Using window.openModal()');
      ...
      setTimeout(() => {
        ...
        console.log('[SHSStudentRegistryModal] After openModal - display:', finalDisplay, 'opacity:', finalOpacity);
        if (finalDisplay === 'none' || finalDisplay === '') {
          console.error('[SHSStudentRegistryModal] ❌ Modal still hidden!');
        }
      }, 100);
    } else {
      console.log('[SHSStudentRegistryModal] window.openModal not available, using fallback');
      // Fallback to direct manipulation
      ...
      requestAnimationFrame(() => {
        ...
        console.log('[SHSStudentRegistryModal] Fallback: Added active class');
      });
    }
